...
- Performance improvement in :class:`.DataFrameGroupBy` and :class:`.SeriesGroupBy` reductions for :class:`ArrowDtype` decimal columns (``sum``, ``prod``, ``min``, ``max``, ``mean``, ``var``), for :class:`ArrowDtype` date, time and binary columns (``min``, ``max``), and for Arrow-backed string columns (``min``, ``max``), i.e. :class:`ArrowDtype` and :class:`StringDtype` with ``storage="pyarrow"``, by dispatching to PyArrow's native ``group_by`` instead of a slower fallback and placing the results with PyArrow's ``take`` rather than converting them to NumPy and back (:issue:`63416`, :issue:`66625`, :issue:`66626`)
...
- Bug in :meth:`.GroupBy.min` and :meth:`.GroupBy.max` on :class:`ArrowDtype` date, time and binary columns ignoring ``min_count``, returning ``null[pyarrow]`` dtype when every group result was ``NA``, and returning ``binary[pyarrow]`` for ``large_binary`` and ``fixed_size_binary`` columns instead of the column's dtype; with ``min_count`` and PyArrow < 24, these methods also returned corrupted values on Arrow-backed string and binary columns with more than 32,768 groups (:issue:`66626`)
