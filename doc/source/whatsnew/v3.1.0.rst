...
- Performance improvement in :meth:`Series.str.normalize` with ``form="NFD"`` or ``form="NFKD"`` for :class:`StringDtype` with storage ``"pyarrow"``, which fell back to an object-dtype implementation instead of using PyArrow's ``utf8_normalize`` kernel (:issue:`XXXXX`)
- Performance improvement in :meth:`Series.str.zfill` for :class:`StringDtype` with storage ``"pyarrow"``, which fell back to an object-dtype implementation instead of using PyArrow's ``utf8_zfill`` kernel (:issue:`XXXXX`)
