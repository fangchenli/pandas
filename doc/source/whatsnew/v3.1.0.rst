...
- Performance improvement in :meth:`Index.join` with ``how="inner"`` and :meth:`Index.intersection` for monotonic numeric indexes of at least one million rows, which are now merged across several threads; the worker count follows the ``mode.max_threads`` option (:issue:`51364`)
