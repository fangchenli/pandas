...
- Performance improvement in :meth:`DataFrame.join`, :func:`merge`, :meth:`DataFrame.reindex` and other operations that gather rows, which split gathers of at least one million elements across threads; the worker count follows the ``mode.max_threads`` option (:issue:`51364`)
...
- Performance improvement in :meth:`Index.intersection` for monotonic numeric indexes, which drop duplicates from the merge output with a linear scan instead of building a hash table, and which deduplicate before converting the result back to nullable or PyArrow-backed dtypes (:issue:`51364`)
...
- Performance improvement in :meth:`Index.join` with ``how="inner"`` for monotonic numeric indexes of at least one million rows, which are now merged across several threads; the worker count follows the ``mode.max_threads`` option (:issue:`51364`)
- Performance improvement in :meth:`Index.join`, :meth:`Index.union` and :meth:`Index.intersection` for monotonic non-object indexes: the ``libjoin`` merge loops now release the GIL, so concurrent calls from multiple threads scale across cores (:issue:`51364`)
