...
- Performance improvement in :meth:`Index.intersection` for monotonic numeric indexes, which drop duplicates from the merge output with a linear scan instead of building a hash table; the merge emits its keys in order, so any duplicates are already adjacent (:issue:`51364`)
