...
Numeric
^^^^^^^
- Bug in :class:`ArrowDtype` boolean data being treated as non-numeric, so it was silently dropped from ``numeric_only=True`` reductions (e.g. :meth:`DataFrame.mean`, :meth:`DataFrame.sum`, :meth:`DataFrame.corr`) and :meth:`Series.std`, :meth:`Series.var`, :meth:`Series.median`, and :meth:`Series.sem` raised ``TypeError``; booleans are now treated as numeric and reduce as ``0``/``1``, matching the NumPy ``bool`` and :class:`BooleanDtype` backends (:issue:`XXXXX`)
...
- Bug in :meth:`Series.to_numpy` on nullable boolean data (:class:`BooleanDtype` or ``bool[pyarrow]``) returning an ``object`` array even when an explicit non-missing ``na_value`` was supplied; the result now uses a ``bool`` NumPy dtype since no missing value survives (:issue:`XXXXX`)
