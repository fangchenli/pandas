...
- Bug in :meth:`.GroupBy.std`, :meth:`.GroupBy.sem`, :meth:`.GroupBy.skew`, :meth:`.GroupBy.any`, and :meth:`.GroupBy.all` raising ``NotImplementedError`` with an empty message on unsupported :class:`ArrowDtype` columns, such as ``binary``, ``date32``, ``time64``, ``dictionary``, and ``list``; the error now names the offending dtype (:issue:`XXXXX`)
