...
- Performance improvement in :meth:`Index.intersection` for monotonic NumPy and nullable numeric indexes, which drop duplicates from the merge output with a linear scan instead of building a hash table, and which deduplicate before converting the result back to nullable dtypes (:issue:`00000`)
- Performance improvement in :meth:`Index.intersection` for monotonic PyArrow-backed numeric indexes, which drop duplicates with PyArrow's run-end encoding instead of building a hash table (:issue:`00001`)
