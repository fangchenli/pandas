...
def benchmark_multiple_files(data_dir: str) -> list[BenchmarkResult]:
    """
    ...
    pattern = os.path.join(data_dir, "yellow_tripdata_*.parquet")
    total_rows = sum(get_row_count(os.path.join(data_dir, f)) for f in TAXI_FILES)
    ...
    for limit_size in [100, 1000]:
        ...
        def full_read_op():
            dfs = [pd.read_parquet(os.path.join(data_dir, f)) for f in TAXI_FILES]
...
def run_all_benchmarks() -> list[BenchmarkResult]:
    ...
    single_file = os.path.join(DATA_DIR, TAXI_FILES[0])
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DATA_DIR) or not TAXI_FILES:
