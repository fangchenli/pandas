...
| AG18 | **Arrow/Acero** Substrait consumer doesn't support `precision_timestamp` (the type/literal that **deprecated** legacy `timestamp`) — `substrait literal did not have any literal type set`; plus **function-coverage** gaps (`No conversion function … ends_with/date_part/starts_with/…`) and cross/non-equi JoinRel rejection. Surfaced *under* AG17 once output_type was fixed | **candidate — characterized on pinned pyarrow 23.0.1; MUST re-verify on latest pyarrow before filing (may be fixed)**; documented in AG17 §residual + `../SUBSTRAIT_ROUNDTRIP.md` | 3 | (in `AG17-...md` §residual) |
