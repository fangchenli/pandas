...
| AG17 | **DataFusion** `to_substrait_plan` omits the **required** `ScalarFunction.output_type` (comparison + arithmetic, DataFrame **and** SQL) → its Substrait roundtrips to *itself* (22/22 TPC-H) but is unconsumable by Acero (0/22; blank-typed error). Causally proven (inject the field → Acero consumes). **Regression of two closed fixes #15831/#20597** (AG11 sibling-miss pattern). Adjacent: producer also leaves aggregate phase `UNSPECIFIED` (Acero rejects). **Local workaround `substrait_fixup.py` → Acero 0/22→7/22** | **found by the Substrait roundtrip-survival probe (`substrait_roundtrip.py`) + root-caused + causally proven + workaround-verified on datafusion 51.0.0 + CONFIRMED on latest 54.0.0 — hand-off-ready** | **2** | `AG17-datafusion-substrait-scalarfunction-output-type.md` |
| AG19 | **Arrow** Substrait **consumer** has no function-extension mapping for `starts_with`/`ends_with`/`regexp_like`/`substring`/`date_part` → `No conversion function … to an Arrow call expression` (blocks 8/22 TPC-H `LIKE`/substring/extract). **Kernels already exist** (`pc.starts_with` etc.) — pure mapping gap, small additive fix. AG9/AG11 sibling-miss of closed #13285. Adjacent: `count(distinct)` DISTINCT invocation unsupported | **found by the Substrait probe + pinned to a per-function coverage matrix (`substrait_fn_coverage.py`) + kernels-exist-confirmed + reproduces on pyarrow 23.0.1 AND 25.0.0 — hand-off-ready** | 3 | `AG19-arrow-substrait-consumer-function-coverage.md` |
| AG18 | **Arrow/Acero** Substrait consumer lags the spec's deprecated→new field moves: (a) **silent-wrong** — reads deprecated `FetchRel.count` (0) while DataFusion writes only newer `count_expr`, so **every `LIMIT` query silently returns 0 rows** (TPC-H q3/q10/q18/q21); (b) no `precision_timestamp` support (`literal did not have any literal type set`). Plus **function-coverage** gaps (`No conversion function … ends_with/date_part/starts_with/…`, 8) and cross/non-equi JoinRel rejection (3). Surfaced *under* AG17; all worked around in `substrait_fixup.py` (0→11/22) except the coverage/join ones | **CONFIRMED on latest pyarrow 25.0.0 too (not just pinned 23.0.1) via `verify_substrait_latest.py` — both manifestations survive**; documented in AG17 §residual + `../SUBSTRAIT_ROUNDTRIP.md` | 3 | (in `AG17-...md` §residual) |
