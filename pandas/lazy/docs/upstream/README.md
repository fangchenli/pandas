...
| AG15 | **NumPy** `StringDType(na_object=…)` — `np.strings` ufuncs handle NA **inconsistently** (propagate / clean-error / **leaky-error** / bool); case ops (`upper`/`lower`/`capitalize`/`swapcase`/`title`) leak `descriptor 'upper' … doesn't apply to a 'float'` via the legacy `_vec_string` path where siblings propagate | **found by the NumPy/CPython scout + characterized on numpy 2.5.1 + non-dup — hand-off-ready** | 3 | `AG15-numpy-stringdtype-na-ufuncs.md` |
