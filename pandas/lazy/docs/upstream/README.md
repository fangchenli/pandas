...
| AG13 | **DataFusion-Python** `register_record_batches` **panics** on an empty table (unchecked `partitions[0][0]` at `context.rs:835`; reachable via `register(name, [tbl.to_batches()])` when `tbl` is empty) | **found by the differential probe + verified on latest 54.0.0 + root-caused + non-dup (sibling of fixed #575) — file-ready** | **2** | `AG13-datafusion-register-empty-panic.md` |
| AG14 | **DataFusion** no common-subplan CSE (recomputes reused subtrees) — **correctness** angle: recomputed non-deterministic float `SUM` under exact `==` gives wrong results (TPC-H q15 → 0 rows) | **tracked upstream — EPIC #22676 + #8777 open; attach correctness data point, don't file** | 3 | `AG14-datafusion-subplan-cse.md` |
