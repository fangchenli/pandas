...
| AG9 | Arrow `string_view` kernel coverage (contribution) | **active contribution — OUR PR #50224 merged 2026-07-07** (Grouper view keys); **OUR PR #50479 open/in review** (predicate kernels, see AG9-next); #44336 open, #50166 + #49964 merged | 3 | `AG9-string-view-kernels.md` |
| AG9-next | Arrow view support for **scalar string predicate kernels** (LIKE/substring/prefix) — the AG9 follow-up PR (take/filter #50164 was already claimed) | **FILED 2026-07-11 — OUR PR #50479 (GH-50478) OPEN, MERGEABLE, in review; `zanmato1984` engaged 07-13, his one ask (utf8_view non-ASCII `ignore_case` test) addressed 07-14 in `8082d31`; Copilot null-slot/OOB concern addressed, re-reviewed clean 07-14. No approval yet.** Re-checked 07-24: still OPEN/MERGEABLE; `zanmato1984` left one **optional** test nit 07-14 18:00 (also exercise a numeric-output view kernel in the null-slot regression) — not a blocker. Feature confirmed NOT yet on Arrow `main` | 3 | `AG9-next-string-predicate-kernels.md` |
...
| AG13 | **DataFusion-Python** `register_record_batches` **panics** on an empty table (unchecked `partitions[0][0]` at `context.rs:835`; reachable via `register(name, [tbl.to_batches()])` when `tbl` is empty) | **✅ MERGED — PR #1627 merged 2026-07-14, issue #1626 closed** (apache/datafusion-python). Keep our empty-frame guard until it ships in a released dep | **2** | `AG13-datafusion-register-empty-panic.md` |
...
| AG19 | **Arrow** Substrait **consumer** has no mapping for the **standard** string functions `starts_with`/`ends_with`/`substring` → `No conversion function …` (blocks TPC-H `LIKE`/substring; unmapped even when anchored). **Kernels exist** (`pc.starts_with` etc.) — small additive registry fix; AG9/AG11 sibling-miss of closed #13285. Adjacent Acero gap: `count(distinct)` DISTINCT invocation. **Spec-split (review):** `date_part`/`regexp_like` are NOT standard Substrait (canonical `extract`/`regexp_match_substring`) → **DataFusion producer** issue, not Arrow; + DataFusion emits all fns unanchored (`urn_reference` unset) | **found by the Substrait probe + per-function coverage matrix (`substrait_fn_coverage.py`) + spec-scope-checked + attribution-tested + reproduces on pyarrow 23.0.1 AND 25.0.0 (releases) — hand-off-ready; **✅ `main`-first gate CLOSED 2026-07-24: CONFIRMED LIVE at Arrow HEAD `62d2dd8270` (`extension_set.cc` `DefaultExtensionIdRegistry` registers only `concat` under the string-fns URI; no starts_with/ends_with/substring). Non-dup (#13285 stale-closed 2025-12-05). Filing-ready pending go-ahead**** | 3 | `AG19-arrow-substrait-consumer-function-coverage.md` |
| AG18 | **Arrow/Acero** consumer doesn't follow the Substrait spec (spec is correct — verified vs substrait-io/substrait): (a) **silent-wrong** — `FetchRel.count` is a `oneof count_mode`; spec (PR #748 + proto docs) says check the oneof, unset = ALL. DataFusion sets `count_expr`; Acero ignores the oneof, reads deprecated `count`=0, treats as `LIMIT 0` → **every `LIMIT` query silently returns 0 rows** (TPC-H q3/q10/q18/q21); (b) ~~no `precision_timestamp` support~~ **RESOLVED at HEAD — drop**. Plus **function-coverage** gaps (standard `starts_with`/`ends_with`/`substring`, now tracked as AG19) and cross/non-equi JoinRel rejection. All worked around in `substrait_fixup.py` (0→11/22) | **✅ `main`-first gate CLOSED 2026-07-24 vs Arrow HEAD `62d2dd8270`: (a) FetchRel silent-`LIMIT 0` CONFIRMED LIVE (`relation_internal.cc:782-786` reads deprecated `fetch.count()`, ignores `count_mode` oneof); (b) `precision_timestamp` RESOLVED upstream (`type_internal.cc:136-145` — AG17-class mirage, dropped). Non-dup. Root cause = a **vendored-proto version lag** (Arrow pins substrait v0.44.0; DataFusion emits the newer forms from v0.63.0). **✅ FILED 2026-07-25 — OUR PR #50635 (GH-50634) OPEN/draft, "Critical Fix":** bumps the proto v0.44→v0.63 and fixes the whole dropped-field family in one go — FetchRel silent-`LIMIT 0` (AG18a) **+ a new sibling: AggregateRel GROUP BY keys dropped** (`expression_references`) collapsing all rows into one group **+** JoinRel enum rename. No reviews yet** | **1** | `AG18-arrow-substrait-fetchrel-limit.md` |
