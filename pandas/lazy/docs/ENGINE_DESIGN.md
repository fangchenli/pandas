...
| M2 ⏳ | Decision layer (**part 1 landed June 2026**) | `engine/decisions.py` annotates the compiled graph: per-column backends per pipeline, explicit conversion points, sink decisions — all visible in `explain(physical=True)`. First migrated decision: groupby backend planned from the input schema (the June routing-bug site), runtime rule kept as fallback; A/B-verified identical. Remaining: migrate filter/sort/join decisions, absorb thresholds into the cost model | gates: 1,544 tests green, baselines unchanged; explain shows decisions |
...
| M4 ⏳ | Parallel sinks (**sort part landed June 2026**) | k-way segment merge replaces the cascading pairwise merge in `_parallel_argsort`: pivot splits with stability-preserving tie handling (all pivot-equal values left of the boundary; run-order gathering makes local stable argsort honor global (run, position) tie order), parallel per-segment merge. 10M argsort 1449→399 ms (3.63x vs cascade era 1.8x); end-to-end sort @10M 1631→856 ms — inside Polars' own range; gates recorded -33% to -57% on every sort shape (multikey 10M 5208→2230 ms). Stability pinned on dup-heavy/NaN/all-equal/sorted/reversed inputs. **Aggregate part resolved by measurement, not machinery**: acero's hash aggregation is already internally multi-threaded — a Python partial-agg sink would fight it, not help it, so the canonical thread-local-partials design (written for engines that own their hash tables) deliberately does NOT apply here. The measured lever was **key representation**: acero aggregates dictionary keys at 13 ms vs 67 ms for raw large_string @10M (faster than Polars' 18 ms) — but Categorical columns were falling through to per-query object materialization (313 ms!). Fixed end-to-end: extract_array wraps Categorical zero-copy as pa.DictionaryArray, the schema maps CategoricalDtype to arrow storage so the decision layer routes groupby[arrow], output converts back to category dtype. **10M category-key groupby: 313 → 21 ms (15x), at parity with Polars.** Remaining known gap: acero's raw-string hashing (67 vs 18 ms) is a kernel-quality gap; cached dictionary encoding per source is the noted future lever | sort target ≥0.5x: **hit**; category-key aggregation: **parity** |
