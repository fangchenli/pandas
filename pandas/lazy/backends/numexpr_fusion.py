"""
NumExpr-based expression fusion for lazy pandas.

This module provides expression fusion capabilities using NumExpr,
which can JIT-compile arithmetic expressions for significant speedups
on large arrays (>100K elements).
...
Usage
-----
The fusion system works in two steps:

1. Detect fuseable expression trees in IR:
   ```
...
2. Or use the high-level evaluator:
   ```
   from pandas.lazy.backends.numexpr_fusion import NumExprEvaluator

   evaluator = NumExprEvaluator(arrays, min_elements=100_000)
   result = evaluator.evaluate(ir_node)
   ```
"""
